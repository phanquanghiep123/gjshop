  ...
  <div class="panel panel-visible" id="spy5">
    <div class="panel-heading">
		<div class="panel-title hidden-xs">
			<span class="glyphicon glyphicon-tasks"></span>Registered Faqs
		</div>
    </div>
            ...
            <tbody>
                @foreach ($faqs as $faq)
	                <tr>
						...
						<td>{!! $faq->answer !!}</td>
						<td>{!! $faq->order !!}</td>
           				<td>{!! $faq->status == 1 ? '<button type="button" class="btn btn-rounded btn-xs btn-success btn-block">Active</button> ' : '<button type="button" class="btn btn-rounded btn-xs btn-danger btn-block">Inactive</button>' !!} </td>
						<td>{!! date('D, d M Y - H:i:s',strtotime( $faq->created_at )) !!}</td>
	                    <td>
	                        <div class="actions pull-right">
	                            <div class="btn-group">
	                              ...
	                              <ul class="dropdown-menu" role="menu">
	                                <li>
	                                    <a href="{!! route('gjadmin.faqs.edit', $faq->id) !!}"> 
	                                        <i class="i"></i> <i class="fa fa-pencil"></i> Edit Faq
	                                    </a>
	                                </li>
	                                <li class="divider"></li>
	                                <li>
	                                    {!! Form::open(['method' => 'DELETE', 'route' => ['gjadmin.faqs.destroy', $faq->id]]) !!}                                    
